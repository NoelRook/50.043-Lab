...
public class LockManager {
    ...
    public synchronized void acquireLock(TransactionId tid, PageId pid, Permissions perm) 
        ...
        // Check if transaction already holds the required lock or a stronger lock
        if (holdsLock(tid, pid, perm)) {
            return;
        }
        
        // If requesting read lock but already have write lock, no need to do anything
        if (perm == Permissions.READ_ONLY && holdsLock(tid, pid, Permissions.READ_WRITE)) {
            return;
        }
        ...
        // Lock cannot be granted immediately, check for deadlock before waiting
        ...
        if (hasCycle()) {
            // Remove from wait-for graph and abort
        ...
        waitingQueue.computeIfAbsent(pid, k -> new LinkedList<>())
                   .offer(new LockRequest(tid, pid, perm));
        
        // Wait for lock to be available
        while (!canGrantLock(tid, pid, perm)) {
            try {
                wait(50); // Wait for 50ms before checking again
            } catch (InterruptedException e) {
                // Clean up and abort
                cleanupWaitingTransaction(tid, pid);
                Thread.currentThread().interrupt();
                throw new TransactionAbortedException(
                    String.format("Transaction %s interrupted while waiting for %s lock on page %s",
                        tid, perm == Permissions.READ_ONLY ? "shared" : "exclusive", pid));
            }
        }
        
        // Remove from waiting structures and grant lock
        cleanupWaitingTransaction(tid, pid);
        grantLock(tid, pid, perm);
